...
## Abstract
High-quality supervised training data form the foundation of reliable intelligent systems in knowledge-intensive and safety-critical domains such as healthcare, intelligent transportation, smart manufacturing, cyber--physical systems, and scientific discovery.
Yet, constructing such data remains critically dependent on domain experts, whose nuanced judgment is irreplaceable in tasks that require deep domain knowledge, contextual reasoning, and adherence to professional standards. To bridge the gap between expert insight and scalable data production, a growing body of research has developed the Expert-in-the-Loop (EITL) paradigm, which integrates human expertise with intelligent algorithms and collaborative platforms throughout the data life cycle.
In this survey, we present a unified, systems-oriented analysis of EITL methodologies along two orthogonal dimensions: data-type evolution and collaboration paradigms. We systematically categorize existing approaches according to the roles of experts and the augmentation mechanisms of intelligent systems, ranging from rule-based preliminary labeling and active sampling to generative synthesis and automated conflict resolution. By abstracting common architectural patterns and interaction mechanisms, this survey establishes a structured taxonomy and conceptual framework for expert-guided data engineering. It further outlines open challenges and future directions for scalable, reliable, and human-centered data construction in modern AI training paradigms.

## Introduction
With the continuous advancement of large language models (LLMs), their applications in natural language understanding, content generation, intelligent dialogue, educational assistance, and cross-modal retrieval have become increasingly widespread, demonstrating strong general-purpose capabilities. However, large models still face substantial performance challenges in specialized domains such as healthcare, law, and finance. These fields demand high levels of accuracy, rigor, and interpretability, yet existing training data often suffer from poor quality, inconsistent terminology, and a lack of contextual depth, leading to limited comprehension, unreliable reasoning, and potentially misleading outputs. A fundamental cause of these limitations lies in the pretraining paradigm of most large models: they are typically pretrained on massive volumes of unsupervised general-domain data, which lack structured semantic labels and depth of domain-specific knowledge. In contrast, supervised training data provide explicit input--output mappings that effectively guide models in learning complex semantics and logical rules. More importantly, when such data are systematically annotated by experts, their accuracy and reliability are significantly enhanced. Experts leverage professional judgment to build high-quality, knowledge-aligned corpora. Training data generated through extensive expert involvement not only serve as reliable supervision signals during model fine-tuning but also lay the foundation for trustworthy deployment in high-stakes environments. Therefore, building a high-quality supervised data ecosystem centered on expert annotation has become a critical pathway for advancing large models toward specialization, precision, and reliability.
While high-quality expert-annotated data are critical for enhancing LLMs' domain-specific capabilities, fully manual annotation faces substantial practical hurdles. First, it relies heavily on specialized expertise and human judgment, rendering it time-consuming, costly, and difficult to scale to large datasets. Second, the lack of standardized protocols and efficient conflict-resolution mechanisms further limits scalability. Resolving annotation conflicts through manual discussion or hierarchical review requires repeated communication and validation, further delaying data production.
These issues collectively create a severe shortfall in the supply of high-quality annotated data relative to the growing demand from LLMs, forming a ``data bottleneck.'' A limited volume of curated data cannot fully unlock the potential of LLMs, hindering their effective optimization and reliable deployment in specialized domains. Thus, improving annotation efficiency while preserving quality and enabling automated or semiautomated conflict resolution are key challenges in scaling high-quality training data production.

Building on the limitations of fully manual and fully automated approaches, the Expert-in-the-Loop (EITL) paradigm has gradually emerged and gained increasing attention. This approach integrates experts as core participants throughout the data creation pipeline---spanning data sampling, annotation, validation, and iterative refinement---thereby enabling dynamic feedback, real-time correction, and task-driven guidance through continuous human--AI collaboration. Compared with fully automated or purely manual annotation, EITL enhances data credibility and semantic accuracy while helping to identify and mitigate model biases and improve the relevance of data to target tasks. Within this framework, expert knowledge is transformed into actionable supervision signals that effectively guide data generation, thereby supporting more efficient and precise fine-tuning of large models.

Despite these advances in data construction paradigms, existing reviews have primarily addressed data processing and data augmentation but lack a systematic synthesis of the methods and platforms used to generate high-quality training data with expert involvement. We review advances in high-quality training data generation under the EITL paradigm along two dimensions: methodology and tooling. By analyzing method--tool coupling mechanisms, with particular emphasis on the \textbf{instantiation of methods within platforms}, we characterize the technical ecosystem of human--AI collaborative data generation. We also discuss current challenges and potential research directions, offering theoretical support and practical guidance for the scalable, domain-specific production of high-quality training data.

Section~\ref{sec:background} presents the background to this review, focusing on the definition of the EITL paradigm and the limitations of existing surveys. Section~\ref{sec:taxonomy} proposes a unified typology that categorizes current expert--AI collaboration methods and platforms according to the type of data being generated. Sections~\ref{sec:supervised}--\ref{sec:preference} analyze how intelligent generation methods and data engineering platforms address the challenges associated with each data type. Section~\ref{sec:case-study} presents a case study that illustrates the corresponding data-generation processes. Finally, Section~\ref{sec:challenges} discusses current challenges and future directions for data generation under the EITL paradigm and provides a concluding synthesis.

## Background
HHuman-in-the-loop (HITL) is a foundational paradigm in machine learning that integrates human input throughout the training, refinement, and evaluation of AI systems, playing a pivotal role in the creation of high-quality training data. Traditionally, HITL frameworks treat human participants as generic annotators or end users whose primary functions are to provide labels or feedback based on surface-level cues. However, this view overlooks a crucial distinction: not all human input is equivalent. In knowledge-intensive domains, the involvement of \emph{experts}---professionals with deep conceptual understanding and procedural expertise---is essential for ensuring both the accuracy and epistemic validity of AI-driven decisions.

This motivates the shift toward Expert-in-the-Loop (EITL), a specialized form of HITL in which expert judgment shapes the semantics, structure, and reliability of training data. As shown in Fig.~\ref{fig:eitl}, unlike conventional labeling, EITL enables the construction of \emph{knowledge-grounded datasets}, in which annotations capture not only \emph{what} is observed but also \emph{why} it is significant, embedding causal reasoning, contextual nuance, and domain-specific uncertainty.

Consequently, methods and platforms designed for EITL require evaluation beyond standard performance metrics. They should be assessed from the perspective of domain experts, with attention to scientific soundness, practical usability, and model explainability. Only through such holistic analysis can we understand how AI tools can effectively augment expert cognition and ultimately improve the efficiency and integrity of knowledge-intensive data-curation pipelines.
## Taxonomy
To systematically address the gaps identified in prior studies, we propose a unified taxonomy that categorizes current approaches to EITL-based data generation according to the type of training data being produced and the expert--AI collaboration mechanism. We distinguish three core categories---supervised data, instruction-following data, and preference data---and analyze each across levels of AI agency in terms of methodological principles, platform requirements, and workflow patterns.
### Overview of the Taxonomy
We present a systematic taxonomy of methodologies for expert-involved dataset construction. The framework is structured along two orthogonal dimensions: Data-Type Evolution, representing the increasing abstraction of supervision signals, and Increasing AI Agency, illustrating the paradigm shift in human--AI collaboration. This taxonomy highlights the transition from labor-intensive manual annotation to scalable, AI-driven curation strategies guided by expert knowledge.
### Evolution of Data Type
From the perspective of data representation, under the EITL framework, experts enhance the quality of supervised, instruction-following, and preference data by injecting deep domain knowledge. They provide accurate labels, author semantically rich instructions, and deliver nuanced feedback, ensuring that training signals reflect not only patterns but also professional reasoning and judgment.
### Collaboration Paradigm

For each data type, we compare EITL paradigms across levels of AI agency, showing how experts' roles evolve from direct annotators to governance-oriented supervisors.

#### Pure Expert

This paradigm represents the traditional approach in which data quality relies solely on human cognition. It is characterized by high accuracy but low scalability. In this part of the review, we focus on methods and platforms for improving the efficiency of expert annotation, such as expert crowdsourcing and arbitration protocols. These approaches primarily aim to enhance the efficiency and accuracy of collaboration among experts rather than to process the data themselves.

#### AI-Guided Expert

With technological advances, it has become clear that relying solely on experts for annotation imposes substantial efficiency limitations. Consequently, a series of data-filtering methods have emerged to assist experts in constructing high-quality datasets. At this stage, AI acts as a filter or assistant to optimize human effort, shifting the focus from exhaustive annotation to high-value annotation. Despite the resulting efficiency gains, experts remain the primary producers of data at this stage.

#### Expert-Guided AI

As intelligent models have become more capable, particularly with the rise of LLMs, they have assumed a greater share of the functions previously performed by experts and have become the primary producers of data. In this stage, experts mainly define the underlying logic or constraints, while AI is responsible for large-scale data generation.
...
### Summary
This survey is organized along two complementary dimensions to provide a clear, extensible, and navigable map of the field: (i) an orthogonal taxonomy grounded in data type $\times$ collaboration paradigm, which structures the problem space and research landscape; and (ii) a methodological and platform-oriented lens that summarizes the end-to-end solution chain from algorithmic design to system deployment, enabling a holistic synthesis of prior work.
